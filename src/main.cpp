...
#if defined(NDEBUG)
# error "Cointest cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2d82436fb086fd52fdd186a62925c6972b07620d542f2b50cb9374f932588dc5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cointest: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cointest Signed Message:\n";
...
{
    // Cointest: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Cointest: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Cointest: 1 day
static const int64 nTargetSpacing = 2 * 60; // Cointest: 2. minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cointest: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cointest: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2d82436fb086fd52fdd186a62925c6972b07620d542f2b50cb9374f932588dc5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Cointest launch date";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1423668600; //epochtime
        ...
        assert(block.hashMerkleRoot == uint256("0x9d602a173a27fd430278d3096ae467196ddfdef357798a6fb861586e8b60a3b1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Cointest: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CointestMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CointestMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CointestMiner(CWallet *pwallet)
{
    printf("CointestMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cointest-miner");
    ...
    try { loop {
        ...
        printf("Running CointestMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CointestMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CointestMiner, pwallet));
}
